...
| Z2 | **Walls and site forces act; a missed link is diagnosed.** The entry on the frontage, the garage at the kerb and the diwaniya's street door are walls: on the kerb line inside the setback, sliding along it, never off it. S1 to S6 from forces.md given coordinates on the plot, the privacy gradient pushing private rooms away from the street, every bubble inside the buildable line; the environmental slider waits for milestone 3. Links strong, the overlap wall stronger. The first arrangement is derived from the program, never random. Auxiliary rooms ride their owner's perimeter. The brief is checked before settling: planarity, links per room against its wall at target aspect, kerb conflicts, each reported with the fix. After settling, any linked pair not touching is corrected by walking the smaller round the larger to the nearest free wall. The owner's O7 corrections are applied on the way. | Z1, O7 | Raising the site weight moves the garage to a street edge in the test. The entry cannot be dragged off the kerb. The same program settles to the same picture twice from a fresh project. A brief asking a 5 m² WC to touch four rooms says so on the Requirements screen. In the feasible test suite every link is touching after settle. | proposed |
| Z3 | **The hallway as an ellipse.** A hallway bubble is an elongated ellipse of its area, long axis along the rooms it serves, turning under the forces; a stair stays a circle. | Z1 | The ellipse lies along its served rooms after settling; its drawn area equals its target. | proposed |
| Z4 | **The morph, as a proposal.** "Lay out from bubbles" is replaced. Each circle inflates in place into a rectangle of its area within the proportion range of its kind, on the nearest grid; the ellipse into a corridor; links into doors; then the G4 joins and alignment tidy the walls. A touching linked pair moves as one body while overlaps are pushed apart, so the morph may reshape but never separates; auxiliaries morph with their owner. Every link ends realized as a door, or unrealized with the tension line and a sentence naming what is in the way. Drawn as an animation of about a second with the camera held. The result sits as a proposal, dashed, with Accept and Back to bubbles: Accept commits the store once, so one undo returns to the bubbles; Back plays the morph in reverse and writes nothing. Rooms past the buildable line are drawn hatched outside it with the overflow in m². | Z2, Z3, G4, O2 | The default program morphs with no two rooms overlapping and every room's centre within 1 m of its bubble. Morphing twice without a change gives the same plan. In the feasible test suite every link is a door after the morph. Back leaves the store untouched. A program that spills shows the hatched rooms and the number rather than a refusal. | proposed |
