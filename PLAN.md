...
| Z4 | **The morph, as a proposal.** "Lay out from bubbles" is replaced. The zones partition the whole buildable area, each point to the nearest bubble with bigger bubbles claiming more, so there are no gaps or overlaps and every touching pair shares a wall; boundaries straightened to the grid into orthogonal polygons of at most eight corners; each zone carved to its target area with the existing carve, the difference to the hallway, which is drawn last as the spine between the rooms it serves at 1.20 m or more; links into doors; reachability from the entry checked. Drawn as an animation of about a second with the camera held. The result sits as a proposal, dashed, with Accept and Back to bubbles: Accept commits the store once, so one undo returns to the bubbles; Back plays the morph in reverse and writes nothing. Rooms past the buildable line are drawn hatched outside it with the overflow in m². | Z2, Z3, G4, G5, O2 | The default program morphs with no gap and no overlap, every bubble contact a shared wall, every room's centre within 1 m of its bubble. Morphing twice without a change gives the same plan. Every link a door and every room reachable from the entry in the feasible test suite. Back leaves the store untouched. A program that spills shows the hatched rooms and the number rather than a refusal. | proposed |
